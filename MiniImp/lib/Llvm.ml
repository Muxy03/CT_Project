...
let fresh_reg (c : counters) : string = c.reg <- c.reg + 1; Printf.sprintf "%%tmp.%s" (string_of_int c.reg)
let _fresh_label (c : counters) : string = c.label <- c.label + 1; Printf.sprintf "bb_%s" (string_of_int c.label)
...
let collect_vars (cfg : Cfg.cfg) = Hashtbl.fold (fun _ node acc ->
...
let compile_node counters (cfg : Cfg.cfg) outVar id node =
  let label = Printf.sprintf "bb_%d:" id in
    let (last_reg, body_instrs) =
      ...
      | Cfg.Stmt _ ->
          raise ( LlvmError (Printf.sprintf "Llvm.compile_node: node %d contain a complex command (Seq/If/While). The CFG was not flattened correctly." id))
    in
      let jump_instrs =
        match node.Cfg.next with
        | Cfg.EOF ->
            let ret_reg = fresh_reg counters in
              [ Printf.sprintf "  %s = load i64, ptr %%%s" ret_reg outVar; Printf.sprintf "  ret i64 %s" ret_reg ]
        | Cfg.NextBlock nxt -> [ Printf.sprintf "  br label %%bb_%d" nxt ]
        | Cfg.CondSelect (n_true, n_false) -> [ Printf.sprintf "  br i1 %s, label %%bb_%d, label %%bb_%d" last_reg n_true n_false ]
      in
...
let generate_llvm_ir (cfg : Cfg.cfg) inVar outVar =
  let c = make_counters () in
    ...
    let header = ["define i64 @func(i64 %input_val) {"; "entry:"] in
      let allocas = List.map (fun v -> Printf.sprintf "  %%%s = alloca i64" v) all_vars in
        let init_input = [ Printf.sprintf "  store i64 %%input_val, ptr %%%s" inVar ] in
          let jump_to_start = [ Printf.sprintf "  br label %%bb_%d" cfg.Cfg.i ] in
            let sorted_nodes = Hashtbl.fold (fun id node acc -> (id, node) :: acc) cfg.nodes [] |> List.sort (fun (a, _) (b, _) -> compare a b) in
              let body = List.map (fun (id, node) -> String.concat "\n" (compile_node c cfg outVar id node)) sorted_nodes in
                let footer = [ "}" ] in
