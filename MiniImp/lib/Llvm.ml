...
let fresh_reg (c : counters) : string =
  c.reg <- c.reg + 1 ;
  Printf.sprintf "%%tmp.%d" c.reg


let _fresh_label (c : counters) : string =
  c.label <- c.label + 1 ;
...
let rec compile_bexpr counters b =
  match b with
  | Ast.True -> ("true", [])
  | Ast.False -> ("false", [])
  | Ast.Less (e1, e2) ->
...
let collect_vars (cfg : Cfg.cfg) =
  Hashtbl.fold
    (fun _ node acc ->
      match node.Cfg.code with Cfg.Stmt (Ast.Assign (v, _)) -> v :: acc | _ -> acc )
    cfg.nodes []
  ...
  let last_reg, body_instrs =
    ...
    | Cfg.Stmt _ ->
        raise
          (LlvmError
             (Printf.sprintf
                "Llvm.compile_node: node %d contain a complex command (Seq/If/While). The CFG was \
                 not flattened correctly."
                id ) )
  in
  let jump_instrs =
...
let generate_llvm_ir (cfg : Cfg.cfg) inVar outVar =
  ...
  in
  let body =
    List.map (fun (id, node) -> String.concat "\n" (compile_node c cfg outVar id node)) sorted_nodes
  in
  let footer = [ "}" ] in
