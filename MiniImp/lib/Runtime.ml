...
let string_of_value v =
  match v with Int i -> string_of_int i | Bool b -> string_of_bool b | Undefined -> "undefined"
...
(* EVALUATION *)
let rec eval_expr mem e =
  match e with
  | Ast.Var v -> mem_get mem v
  | Ast.Int i -> Int i
  | Ast.Add (e1, e2) -> (
      match (eval_expr mem e1, eval_expr mem e2) with
      | Int a, Int b -> Int (a + b)
...
let rec eval_bexpr mem b =
  match b with
  | Ast.True -> Bool true
  | Ast.False -> Bool false
  | Ast.And (b1, b2) -> (
      match (eval_bexpr mem b1, eval_bexpr mem b2) with
      | Bool v1, Bool v2 -> Bool (v1 && v2)
      | _ -> raise (RuntimeError "Type error in 'and' operation") )
  | Ast.Not b -> (
      match eval_bexpr mem b with
      | Bool v -> Bool (not v)
...
let eval program =
  ...
  | Ast.Program (input_name, output_name, cmd) ->
      ...
      let mem0 =
        ...
        let m2 = mem_set m1 output_name Undefined in
        m2
      in
