...
let def_of_node block =
  match block with Cfg.Stmt (Ast.Assign (v, _)) -> VarSet.singleton v | _ -> VarSet.empty


let use_of_node block =
  match block with
...
let compute_preds (cfg : Cfg.cfg) =
  let init = Hashtbl.fold (fun id _ acc -> NodeMap.add id [] acc) cfg.nodes NodeMap.empty in
  Hashtbl.fold
    (fun id node acc ->
...
let compute_live_variables (cfg : Cfg.cfg) =
  ...
  let initial_wl = Hashtbl.fold (fun id _ acc -> id :: acc) cfg.nodes [] in
  let rec loop wl map =
    match wl with
    | [] -> map
    | n :: rest ->
        ...
        let new_liveOut =
          List.fold_left
            (fun acc succ_id -> VarSet.union acc (NodeMap.find succ_id map).liveIn)
            VarSet.empty (Cfg.successors node)
        in
        let new_liveIn =
          VarSet.union (use_of_node node.Cfg.code)
            (VarSet.diff new_liveOut (def_of_node node.Cfg.code))
        in
        if VarSet.equal state_n.liveIn new_liveIn && VarSet.equal state_n.liveOut new_liveOut then
          loop rest map
        else
...
let compute_reaching_definitions (cfg : Cfg.cfg) : rd_state NodeMap.t =
  ...
  let initial_map =
    Hashtbl.fold (fun id _ acc -> NodeMap.add id empty_rd acc) cfg.nodes NodeMap.empty
  in
  let initial_wl = Hashtbl.fold (fun id _ acc -> id :: acc) cfg.nodes [] in
  let rec loop wl map =
    match wl with
    ...
    | n :: rest ->
        ...
        let new_in =
          List.fold_left
            (fun acc p_id -> DefSet.union acc (NodeMap.find p_id map).rdOut)
            DefSet.empty preds
        in
        ...
        let gen, kill =
          match node.Cfg.code with
          | Cfg.Stmt (Ast.Assign (v, _)) -> (DefSet.singleton n, find_all_defs_of_var cfg v)
...
let check_undefined_variables program (cfg : Cfg.cfg) =
  ...
  let input_var = match program with Ast.Program (name, _, _) -> name in
...
let print_undefined_warnings program cfg =
  ...
  if VarSet.is_empty undef_vars then
    ...
    VarSet.iter
      (fun v -> Printf.printf "WARNING: Variable '%s' may be used uninitialized!\n" v)
      undef_vars
