  ...
  let init = Hashtbl.fold (fun id _ acc -> NodeMap.add id [] acc) cfg.nodes NodeMap.empty in
    Hashtbl.fold (fun id node acc ->
      let add_pred target map =
        ...
        NodeMap.add target (id :: preds) map in
          ...
          | n :: rest ->
              let node    = Cfg.find_node cfg.nodes n in
                let state_n = NodeMap.find n map in
                  let new_liveOut = List.fold_left (fun acc succ_id -> VarSet.union acc (NodeMap.find succ_id map).liveIn) VarSet.empty (Cfg.successors node) in
                    let new_liveIn = VarSet.union (use_of_node node.Cfg.code) (VarSet.diff new_liveOut (def_of_node node.Cfg.code)) in
  ...
  let rec loop wl map = match wl with
    ...
    | n :: rest ->
        ...
        (* IN[n] = union of OUT[pred] *)
          let preds  = NodeMap.find n preds_map in
            let new_in = List.fold_left (fun acc p_id -> DefSet.union acc (NodeMap.find p_id map).rdOut) DefSet.empty preds in

        (* Gen / Kill *)
              let gen, kill = match node.Cfg.code with
