      ...
      Hashtbl.iter (fun id node ->
        ...
        | Cfg.Stmt (Ast.Assign (v, _)) ->
            ...
            if not (DataFlow.VarSet.mem v state.DataFlow.liveOut) && v <> outVar then
              update_node cfg id node (Cfg.Stmt Ast.Skip) changed (Printf.sprintf "  [Dead Store Elim]    nodo %d: rimosso '%s := ...'" id v)
        | _ -> ()
      ) cfg.nodes;
  !changed
...
let constant_folding (cfg : Cfg.cfg) =
  let changed = ref false in
    Hashtbl.iter (fun id node -> match node.Cfg.code with
      | Cfg.Stmt (Ast.Assign (v, e)) -> let fe = fold_expr e in
          if fe <> e then
            update_node cfg id node (Cfg.Stmt (Ast.Assign (v, fe))) changed (Printf.sprintf "  [Constant Folding]   nodo %d: semplificato assegnamento '%s'" id v)
      | Cfg.Condition b ->
          let fb = fold_bexpr b in
            if fb <> b then
              update_node cfg id node (Cfg.Condition fb) changed (Printf.sprintf "  [Constant Folding]   nodo %d: semplificata condizione" id)
      | _ -> ()
    ) cfg.nodes;
  !changed
...
let get_constant_value var inDefs (cfg : Cfg.cfg) =
  let relevant = DataFlow.DefSet.filter (fun d_id -> match (Cfg.find_node cfg.nodes d_id).Cfg.code with
  ...
  in
    if DataFlow.DefSet.is_empty relevant then None
      ...
      let values = DataFlow.DefSet.fold (fun d_id acc ->
...
let rec propagate_in_expr e inDefs (cfg : Cfg.cfg) = match e with
  | Ast.Var v -> (
    match get_constant_value v inDefs cfg with
      | Some c -> Ast.Int c
      | None   -> Ast.Var v
    )
      ...
      let state = DataFlow.NodeMap.find id rd_map in
        ...
        | Cfg.Stmt (Ast.Assign (v, e)) ->
            let ne = propagate_in_expr e state.DataFlow.rdIn cfg in
              if ne <> e then
                update_node cfg id node (Cfg.Stmt (Ast.Assign (v, ne))) changed (Printf.sprintf "  [Constant Prop]      nodo %d: propagato valore in '%s'" id v)
        | Cfg.Condition b ->
            let nb = propagate_in_bexpr b state.DataFlow.rdIn cfg in
              if nb <> b then
                update_node cfg id node (Cfg.Condition nb) changed (Printf.sprintf "  [Constant Prop]      nodo %d: propagato valore in condizione" id)
        | _ -> ()
    ) cfg.nodes;
  !changed
...
let rec optimize_pipeline (cfg : Cfg.cfg) outVar iter =
  ...
  if c1 || c2 || c3 then begin
    ...
    Printf.printf "  -> Fixpoint reached.\n"

let optimize (cfg : Cfg.cfg) outVar = Printf.printf "\n Starting Optimization Pipeline...\n"; optimize_pipeline cfg outVar 1
