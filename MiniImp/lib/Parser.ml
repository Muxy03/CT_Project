(*
Precedence Order: (low -> high = top -> down)
     %right ELSE   (lowest)
     %right DO
     %left  SEMI
     %left  AND
...
let parse (src : string) : Ast.program =
    ...
    in
      ...
      in
        let expect t =
          let got = advance () in
          if got <> t then
            raise (ParseError (Printf.sprintf "expected %s, got %s" (Lexer.string_of_token t) (Lexer.string_of_token got)))
        in
          let expect_var () =
            match advance () with
            | VAR v -> v
  ...
  and parse_mul () =
    let lhs = ref (parse_unary ()) in
      ...
      !lhs
  and parse_unary () = match peek () with
    | MINUS -> ignore (advance ()); let e = parse_unary () in
        Sub (Int 0, e)
    | _ -> parse_atom ()
  and parse_atom () = match advance () with
    | VAR v -> Var v
    | INT i -> Int i
    | LPAREN ->
        let e = parse_expr () in
  ...
  and parse_band () =
    let lhs = ref (parse_bnot ()) in
      let continue_ = ref true in
        while !continue_ do
          match peek () with
          | AND -> ignore (advance ()); let rhs = parse_bnot () in
                lhs := And (!lhs, rhs)
          | _ -> continue_ := false
        done;
      !lhs
  and parse_bnot () =
    match peek () with
    | NOT -> ignore (advance ()); let b = parse_bnot () in
        Not b
    | TRUE -> ignore (advance ());  True
    | FALSE -> ignore (advance ()); False
    | _ -> let e1 = parse_expr () in
            expect LESS;
  ...
  and parse_cmd_atom () = match peek () with
    | LPAREN -> ignore (advance ());
        let c = parse_cmd () in
          expect RPAREN; CmdParen c
    | SKIP  -> ignore (advance ()); Skip
    | IF    -> ignore (advance ());
