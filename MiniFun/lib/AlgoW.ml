...
let counter = ref 0
let fresh_tvar () = incr counter ; TVar (Printf.sprintf "'a %s" (string_of_int !counter))
...
let rec apply_mono (s : subst) (t : mono) : mono =
  ...
  | TVar a -> (
      ...
      | Some t' -> apply_mono s t' (* Recursive search on the chain *)
      | None -> t)
...
let rec fv_mono m = match m with
...
let rec unify t1 t2 =
  ...
  | TVar a, t | t, TVar a -> if occurs a t then failwith "Unification failed: occurs check" else [ (a, t) ]
...
let rec infer env e : subst * mono =
  ...
  | Binop (op, e1, e2) ->
      let s1, t1 = infer env e1 in
        let s2, t2 = infer (apply_env s1 env) e2 in
          let t_left, t_right, t_ret = match op with
              ...
              let env3 = apply_env s3 env2 in
                let s4, t_else = infer env3 e_else in
                  let s5 = unify (apply_mono s4 t_then) t_else in
                    ( compose_subst s5 (compose_subst s4 (compose_subst s3 (compose_subst s2 s1))), apply_mono s5 t_else )
  | Let (x, e1, e2) ->
      let s1, t1 = infer env e1 in
              ...
              let s1, t_body = infer env_body e_body in
                let s2 = unify (apply_mono s1 b) t_body in
                  let s_total1 = compose_subst s2 s1 in
                    ...
                    (* apply s_total1 to the function type before generalizing *)
                      let inferred_fun_type = apply_mono s_total1 fun_type in
                        let poly_f = gener env_in inferred_fun_type in
                          ...
                          (* apply s3 (the substitution of e_in) to the type t_in *)
                            (compose_subst s3 s_total1, apply_mono s3 t_in)


let typecheck env e =
  (* NO reset counter := 0 to avoid conflict with the env *)
  let s, t = infer env e in
    apply_mono s t (* apply the final substitution to the type *)
