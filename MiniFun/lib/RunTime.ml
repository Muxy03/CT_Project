...
let env_get env x =
  ...
  with Not_found -> raise (RunTimeError (Printf.sprintf "Unbound variable: %s" x))

let extend_env env x v =
  let new_env = Hashtbl.copy env in
    Hashtbl.add new_env x v; new_env
  ...
  | Ast.App (t1, t2) -> (
      let v1 = eval env t1 in
        let v2 = eval env t2 in
          match v1 with
          | VClosure (x, body, env1) -> eval (extend_env env1 x v2) body
