...
type token =
  | TRUE | FALSE | FUN | ARROW | TARROW | COLON
  | IF | THEN | ELSE
  | LET | IN | LETFUN
  | EQ
  | PLUS | MINUS | STAR | AND | LT | NOT
  | LPAREN | RPAREN
  | TINT | TBOOL
  | VAR of string
  | INT of int
...
let string_of_token t = match t with
  ...
  | LET -> "let" | IN -> "in" | LETFUN -> "letfun"
  | EQ -> "="
  | PLUS -> "+" | MINUS -> "-" | STAR -> "*" | AND -> "&&" | LT -> "<" | NOT -> "~"
  | LPAREN -> "(" | RPAREN -> ")"
  | TINT -> "int" | TBOOL -> "bool"
  | VAR v -> Printf.sprintf "variable '%s'" v
  | INT i -> Printf.sprintf "integer %s" (string_of_int i)
  | EOF -> "<eof>"
...
let keyword_or_var w = match w with
  | "true" -> TRUE
  | "false" -> FALSE
  | "fun" -> FUN
  | "if" -> IF
...
let tokenize (src : string) : token list =
  ...
  while !pos < n do
    ...
    | ':' -> emit COLON; incr pos
    | '=' ->
        if next_is '>' then begin emit ARROW; pos := !pos + 2 end
        else begin emit EQ; incr pos end
    | '-' ->
        if next_is '>' then begin emit TARROW; pos := !pos + 2 end
        else begin emit MINUS; incr pos end
    | '&' ->
        if next_is '&' then begin emit AND; pos := !pos + 2 end
