...
let parse (src : string) : Ast.expr =
  ...
  let expect_var () =
    ...
    | VAR v -> v
    | t ->
        raise (ParseError (Printf.sprintf "expected identifier, got %s" (Lexer.string_of_token t)))
  in

  (* Type parser: right-associative TARROW (->) *)
  let rec parse_typo () =
  ...
  and parse_typo_atom () =
    ...
    | LPAREN ->
        ...
        t
    | t ->
        raise (ParseError (Printf.sprintf "unexpected token in type: %s" (Lexer.string_of_token t)))
  in

  (* Expression parser *)
  let rec parse_operand () =
    match peek () with
