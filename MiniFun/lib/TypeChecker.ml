...
let rec typecheck (env : env) (e : expr) : typo = match e with
  | Num _ -> Int
  | Boolean _ -> Bool
  | Var x -> (
      try List.assoc x env
      with Not_found -> raise (TypeError (Printf.sprintf "Unbound variable: %s" x))
    )
  | Func (x, Some t_arg, body) ->
      let t_body = typecheck ((x, t_arg) :: env) body in
        Fun (t_arg, t_body)

  | Func (_, None, _) -> raise (TypeError "Required explicit type annotations on Func")
  | App (e1, e2) -> (
      let t1 = typecheck env e1 in
        let t2 = typecheck env e2 in
          match t1 with
          | Fun (t_arg, t_ret) -> expect_type t_arg t2 "function argument"; t_ret
          | _ -> raise (TypeError (Printf.sprintf "Application of non-function: %s" (string_of_typo t1)))
    )
  | Binop (op, e1, e2) ->
      let t1 = typecheck env e1 in
        let t2 = typecheck env e2 in (
  ...
  | Not e1 ->
      let t1 = typecheck env e1 in
        expect_type Bool t1 "logical NOT operator"; Bool
  | If (e1, e2, e3) ->
      let t1 = typecheck env e1 in
        expect_type Bool t1 "if condition";
        let t2 = typecheck env e2 in
          let t3 = typecheck env e3 in
            expect_type t2 t3 "if branches (then and else must match)"; t2
  | Let (x, e1, e2) ->
      let t1 = typecheck env e1 in
  ...
  | LetFun (_, _, Some _, _, _) -> raise (TypeError "letfun annotation must be a function type (e.g., Int -> Int)")
  | LetFun (_, _, None, _, _) -> raise (TypeError "Required explicit type annotations on LetFun")
